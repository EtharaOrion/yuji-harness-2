...
spent a refresh token that had already been rotated.
...
When a refresh happens, we write to (6) only -- never back to Keychain --
because the ``claude`` CLI also manages Keychain and we don't want write
races. Refreshing rotates the refresh token, which the ``claude`` CLI on the
same machine also holds, so the provider re-reads the stores first and refreshes
only when none of them has a live token (the CLI usually refreshed already).
"""
...
_CACHE_PATH = Path.home() / ".cache" / "yuji-ccbridge" / "claude_creds.json"
...
def _read_credentials_file() -> Optional[str]:
    return _read_file(Path.home() / ".claude" / ".credentials.json")
...
def _parse(raw: str) -> OAuthCredentials:
    try:
...
def load_credentials() -> OAuthCredentials:
    ...
    for reader in (
        ...
        if not raw:
            continue
        try:
            found.append(_parse(raw))
        except CredentialsError as e:
            errors.append(str(e))
    if not found:
        if errors:
            raise CredentialsError(errors[0])
        raise CredentialsError(_no_credentials_hint())
    return max(found, key=lambda c: c.expires_at_ms)
    ...
    def get_access_token(self) -> str:
        with self._lock:
            ...
            if self._creds.is_expired():
                # The claude CLI on this machine refreshes the same login and
                # rotates its refresh token when it does. Re-read the stores
                # before refreshing: a live token there costs nothing, and
                # refreshing from our stale in-memory copy would spend a
                # refresh token that may already be dead.
                try:
