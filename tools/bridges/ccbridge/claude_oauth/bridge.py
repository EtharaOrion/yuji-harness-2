...
Vendored into the yuji harness at tools/bridges/ccbridge; see the README there
...
def normalize_body_for_anthropic_direct(body: dict[str, Any]) -> dict[str, Any]:
    """Make a request's thinking shape one Anthropic-direct accepts.

    ADAPTIVE THINKING IS PASSED THROUGH UNCHANGED (the default), including the
    caller's ``display`` choice. Measured 2026-09-22 against api.anthropic.com
    on the OAuth path, claude-opus-5, same prompt, with and without tools:

        thinking={type:adaptive, display:summarized} + output_config.effort
            -> accepted; thinking text 68-76 chars (readable)
    ...
    made the caller's display choice impossible: enabled ignores it.

    ``display`` is left to the caller, because it is a real trade-off and not
    a formatting detail. tools/openhands_agent/README.md records it: readable
    (summarized) thinking, echoed back on later turns, drew refusals in the
    OpenHands agent's context that omitted thinking never did.

    ``output_config`` (effort) is kept with adaptive thinking -- it is that
    mode's control -- and dropped only when converting, as the reference did.
    """
    if not isinstance(body, dict):
        return body
    ...
    if ttype == "adaptive" and _adaptive_thinking_mode() == "passthrough":
        return body

    # The converted/manual shape: the reference's rule, which found readable
    # text needed display=summarized on the models it was measured on.
...
def clamp_thinking_budget(body: dict[str, Any]) -> dict[str, Any]:
    ...
    A budget that does not fit is halved against ``max_tokens`` (never below
    Anthropic's 1024 floor). When even the floor does not fit there is no room
    to think at all, so thinking is dropped rather than sent invalid.
    """
    thinking = body.get("thinking")
    ...
    if not isinstance(max_tokens, int) or max_tokens <= 0:
        return body
    budget = thinking.get("budget_tokens")
    if isinstance(budget, int) and 0 < budget < max_tokens:
        return body
    if max_tokens <= MIN_THINKING_BUDGET:
        _LOG.info(
            "max_tokens=%d leaves no room for a thinking budget; dropping thinking",
            max_tokens,
        )
        body.pop("thinking", None)
        return body
    thinking["budget_tokens"] = max(MIN_THINKING_BUDGET, max_tokens // 2)
    return body
